try:
	import requests,time,json,os
	from requests.structures import CaseInsensitiveDict
except:
	os.system("pip install requests")

#1


api1 = input('Enter Lkey : ')

message1  = input('Enter Message : ')

#2


#api2 = input('Enter Lkey : ')

#message2  = str(input("[>]Enter Your Target Message2:"))

#3

#api3 = input('Enter Lkey : ')

#message3  = str(input("[>]Enter Your Target Message3:"))

#4

#api4 = input('Enter Lkey : ')

#message4  = str(input("[>]Enter Your Target Message4:"))


#5

#api5 = input('Enter Lkey : ')

#message5  = str(input("[>]Enter Your Target Message5:"))

#6

#api6 = input('Enter Lkey : ')

#message6 = str(input("[>]Enter Your Target Message6:"))

#7

#api7 = input('Enter Lkey : ')

#message7 = str(input("[>]Enter Your Target Message7:"))

#api8 = input('Enter Lkey : ')

#message8  = str(input("[>]Enter Your Target Message8:"))

#9

#api9 = input('Enter Lkey : ')

#message9  = str(input("[>]Enter Your Target Message9:"))

#10

#api10 = input('Enter Lkey : ')

#message10  = str(input("[>]Enter Your Target Message10:"))






#MainTools


#amount = int(input('[>] Enter Your Target Amount:'))



headers1 = CaseInsensitiveDict()

headers1["Content-Type"] = "application/json; charset=UTF-8"
headers1["User-Agent"] = "gzip"
headers1["x-app-key"] = "000oc0so48owkw4s0wwo4c00g00804w80gwkw8kg"
headers1["x-api-key"] = api1

url1 = "https://circle.robi.com.bd/mylife/appapi/appcall.php?op=performAction&action=kast&message="+message1

#2



headers2 = CaseInsensitiveDict()

headers2["Content-Type"] = "application/json; charset=UTF-8"
headers2["User-Agent"] = "gzip"
headers2["x-app-key"] = "000oc0so48owkw4s0wwo4c00g00804w80gwkw8kg"
headers2["x-api-key"] = api1

url2 = "https://circle.robi.com.bd/mylife/appapi/appcall.php?op=performAction&action=kast&message="+message1

#3

headers3 = CaseInsensitiveDict()

headers3["Content-Type"] = "application/json; charset=UTF-8"
headers3["User-Agent"] = "gzip"
headers3["x-app-key"] = "000oc0so48owkw4s0wwo4c00g00804w80gwkw8kg"
headers3["x-api-key"] = api1

url3 = "https://circle.robi.com.bd/mylife/appapi/appcall.php?op=performAction&action=kast&message="+message1

#4


headers4 = CaseInsensitiveDict()

headers4["Content-Type"] = "application/json; charset=UTF-8"
headers4["User-Agent"] = "gzip"
headers4["x-app-key"] = "000oc0so48owkw4s0wwo4c00g00804w80gwkw8kg"
headers4["x-api-key"] = api1

url4 = "https://circle.robi.com.bd/mylife/appapi/appcall.php?op=performAction&action=kast&message="+message1


#5


headers5 = CaseInsensitiveDict()

headers5["Content-Type"] = "application/json; charset=UTF-8"
headers5["User-Agent"] = "gzip"
headers5["x-app-key"] = "000oc0so48owkw4s0wwo4c00g00804w80gwkw8kg"
headers5["x-api-key"] = api1

url5 = "https://circle.robi.com.bd/mylife/appapi/appcall.php?op=performAction&action=kast&message="+message1


#6

headers6 = CaseInsensitiveDict()

headers6["Content-Type"] = "application/json; charset=UTF-8"
headers6["User-Agent"] = "gzip"
headers6["x-app-key"] = "000oc0so48owkw4s0wwo4c00g00804w80gwkw8kg"
headers6["x-api-key"] = api1

url6 = "https://circle.robi.com.bd/mylife/appapi/appcall.php?op=performAction&action=kast&message="+message1

#7

headers7 = CaseInsensitiveDict()

headers7["Content-Type"] = "application/json; charset=UTF-8"
headers7["User-Agent"] = "gzip"
headers7["x-app-key"] = "000oc0so48owkw4s0wwo4c00g00804w80gwkw8kg"
headers7["x-api-key"] = api1

url7 = "https://circle.robi.com.bd/mylife/appapi/appcall.php?op=performAction&action=kast&message="+message1

#8


headers8 = CaseInsensitiveDict()

headers8["Content-Type"] = "application/json; charset=UTF-8"
headers8["User-Agent"] = "gzip"
headers8["x-app-key"] = "000oc0so48owkw4s0wwo4c00g00804w80gwkw8kg"
headers8["x-api-key"] = api1

url8 = "https://circle.robi.com.bd/mylife/appapi/appcall.php?op=performAction&action=kast&message="+message1

#9


headers9 = CaseInsensitiveDict()

headers9["Content-Type"] = "application/json; charset=UTF-8"
headers9["User-Agent"] = "gzip"
headers9["x-app-key"] = "000oc0so48owkw4s0wwo4c00g00804w80gwkw8kg"
headers9["x-api-key"] = api1

url9 = "https://circle.robi.com.bd/mylife/appapi/appcall.php?op=performAction&action=kast&message="+message1


#10


headers10 = CaseInsensitiveDict()

headers10["Content-Type"] = "application/json; charset=UTF-8"
headers10["User-Agent"] = "gzip"
headers10["x-app-key"] = "000oc0so48owkw4s0wwo4c00g00804w80gwkw8kg"
headers10["x-api-key"] = api1

url10 = "https://circle.robi.com.bd/mylife/appapi/appcall.php?op=performAction&action=kast&message="+message1


#actionLoop


while 1:
	
	try:
		#1
		#time.sleep(0.1)
		resp1 = requests.post(url1, headers=headers1)
		response= resp1.text 
		json_object = json.loads(response)
		json_formatted_str = json.dumps(json_object, indent=1)
		print(json_formatted_str)
		#2
		#time.sleep(0.1)
		resp2 = requests.post(url2, headers=headers2)
		response= resp2.text 
		json_object = json.loads(response)
		json_formatted_str = json.dumps(json_object, indent=1)
		print(json_formatted_str)
		#3
		#time.sleep(0.1)
		resp3 = requests.post(url3, headers=headers3)
		response= resp3.text 
		json_object = json.loads(response)
		json_formatted_str = json.dumps(json_object, indent=1)
		print(json_formatted_str)
		#4
		#time.sleep(0.1)
		resp4 = requests.post(url4, headers=headers4)
		response= resp4.text 
		json_object = json.loads(response)
		json_formatted_str = json.dumps(json_object, indent=1)
		print(json_formatted_str)
		#5
		#time.sleep(0.1)
		resp5 = requests.post(url5, headers=headers5)
		response= resp5.text 
		json_object = json.loads(response)
		json_formatted_str = json.dumps(json_object, indent=1)
		print(json_formatted_str)
		#6
		#time.sleep(0.1)
		resp6 = requests.post(url6, headers=headers6)
		response= resp6.text 
		json_object = json.loads(response)
		json_formatted_str = json.dumps(json_object, indent=1)
		print(json_formatted_str)
		#7
		#time.sleep(0.1)
		resp7 = requests.post(url7, headers=headers7)
		response= resp7.text 
		json_object = json.loads(response)
		json_formatted_str = json.dumps(json_object, indent=1)
		print(json_formatted_str)
		#8
		#time.sleep(0.1)
		resp8 = requests.post(url8, headers=headers8)
		response= resp8.text 
		json_object = json.loads(response)
		json_formatted_str = json.dumps(json_object, indent=1)
		print(json_formatted_str)
		#9
		#time.sleep(0.1)
		resp9 = requests.post(url9, headers=headers9)
		response= resp9.text 
		json_object = json.loads(response)
		json_formatted_str = json.dumps(json_object, indent=1)
		print(json_formatted_str)
		#10
		#time.sleep(0.1)
		resp10 = requests.post(url10, headers=headers10)
		response= resp10.text 
		json_object = json.loads(response)
		json_formatted_str = json.dumps(json_object, indent=1)
		print(json_formatted_str)
	except:
		print("Server Connection  Error!")
		#print(str(i+1)+' Shout not sent\n')